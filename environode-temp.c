...
PROCESS(button_process, "Button Sensing Assignment");
...
PROCESS_THREAD(button_process, ev, data)
{
  ...
  while(1) {
    ...
    data == &button_digital0_sensor||data == &button_digital1_sensor||data == &button_digital2_sensor||
    data == &button_dry_in_1_sensor||data == &button_dry_in_2_sensor||data == &button_dry_in_3_sensor||
    data == &button_dry_in_4_sensor));
    if(data == &button_user_sensor){
      printf("Germany");
    }
    else if(data == &button_digital0_sensor){
      printf("digital0\r\n");
      READ_INPUT();
    }
    else if(data == &button_digital1_sensor){
      printf("digital1\r\n");
      READ_INPUT();
    }
    else if(data == &button_digital2_sensor){
      printf("digital2\r\n");
      READ_INPUT();
    }
    else if(data == &button_dry_in_1_sensor){
      printf("dryin1\r\n");
      READ_INPUT();
    }
    else if(data == &button_dry_in_2_sensor){
      printf("dryin2\r\n");
      READ_INPUT();
    }
    else if(data == &button_dry_in_3_sensor){
      printf("dryin3\r\n");
      READ_INPUT();
    }
    else if(data == &button_dry_in_4_sensor){
      printf("dryin4\r\n");
      READ_INPUT();
    }
  }
...
PROCESS_THREAD(environode_process, ev, data)
{
  ...
  while(1) {
    ...
    printf("debug4\r\n");
    READ_INPUT();
    sprintf(sensing_payload, "%s%d%s%d%s%d%s%d%s%d%s%d%s%d%s%s%s%s%s", "{\"version\":\"1.0.0\",\"datastreams\" : [ {\"id\" : \"digital1\",\"current_value\" : \"", sensor_cond[0], "\"},{\"id\" : \"digital2\",\"current_value\" : \"", sensor_cond[1], "\"},{\"id\" : \"digital3\",\"current_value\" : \"", sensor_cond[2], "\"},{\"id\" : \"dryin1\",\"current_value\" : \"", sensor_cond[3], "\"},{\"id\" : \"dryin2\",\"current_value\" : \"", sensor_cond[4], "\"},{\"id\" : \"dryin3\",\"current_value\" : \"", sensor_cond[5], "\"},{\"id\" : \"dryin4\",\"current_value\" : \"", sensor_cond[6], "\"},{\"id\" : \"temperature\",\"current_value\" : \"", ctemp, "\"},{\"id\" : \"humidity\",\"current_value\" : \"", chum, "\"}]}");
    printf("%s%d\r\n%s%d\r\n%s%d\r\n%s%d\r\n%s%d\r\n%s%d\r\n%s%d\r\n%s%s\r\n%s%s%s\r\n", "{\"version\":\"1.0.0\",\"datastreams\" : [ {\"id\" : \"digital1\",\"current_value\" : \"", sensor_cond[0], "\"},{\"id\" : \"digital2\",\"current_value\" : \"", sensor_cond[1], "\"},{\"id\" : \"digital3\",\"current_value\" : \"", sensor_cond[2], "\"},{\"id\" : \"dryin1\",\"current_value\" : \"", sensor_cond[3], "\"},{\"id\" : \"dryin2\",\"current_value\" : \"", sensor_cond[4], "\"},{\"id\" : \"dryin3\",\"current_value\" : \"", sensor_cond[5], "\"},{\"id\" : \"dryin4\",\"current_value\" : \"", sensor_cond[6], "\"},{\"id\" : \"temperature\",\"current_value\" : \"", ctemp, "\"},{\"id\" : \"humidity\",\"current_value\" : \"", chum, "\"}]}");
    if(isReady == 0) {
